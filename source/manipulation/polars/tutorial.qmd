...
On peut facilement récupérer le détail des résultats grâce à la méthode `unnest` :
...
- **schéma** : la liste des colonnes est affichée avec des informations très utiles sur le schéma des données :
  ...
  Ceka permet de comprendre la nature des informations stockées dans chaque colonne.
...
La structure pl.when(pl.col.foo > 2).then(1).otherwise(1 + pl.col.bar).alias("val")

Deux types d'expression différents :
- avec alias
- plus similaire au tidyverse, avec assignation.

Attention, les caractères simple sont assimilés aux valeurs des colonnes.
...
Seule `Null` est considéré par défaut comme une valeur manquante.
...
df = pl.DataFrame(
    data = {
        "var1": [1.3, 5.6, np.nan, 1.4, None, np.nan],
        "var2": np.random.randint(-10, 10, 6),
