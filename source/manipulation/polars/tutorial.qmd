...
"TO DO" - Polars Séries
"TO DO" les sélecteurs dans Polars
...
Par exemple, pour filter des données, on devra spécifier les bornes en appelant les fonctions dédiées de Polars ou du module `datetime`.
...
Pour contourner ce problème, il suffit de préférer une méthode de type `join` avec `df1.join(df2, on=df1.columns, how="full", coalesce=True)` (*cf.* ci-après).
